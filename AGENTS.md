...
The **GTO Solver page** (`study-app/src/pages/Solver.tsx`, sidebar "GTO Solver") is a workbench against a UPI bridge: a local HTTP wrapper around a persistent PioSOLVER process. Assumed contract — `POST {url}/upi` with `{"commands": string[]}` returning `{"responses": string[]}` (sequential, same solver process, END markers stripped, `ERROR` lines preserved; bridge must answer CORS preflights; `show_children_actions` normalized to one action token per line, `show_strategy` to one 1326-float line per child in child order). Default endpoint `http://127.0.0.1:8500`, persisted in localStorage. https pages may call http://localhost (potentially-trustworthy origin), so the deployed site can drive a local bridge.
