...
The **GTO Solver page** (`study-app/src/pages/Solver.tsx` + `pages/solver/` — Setup and Solution views, sidebar "GTO Solver") is a workbench against a UPI bridge. Setup configures board / ranges / stacks / per-street-per-position sizings (with localStorage presets); Solution shows the table visualizer, the street action tree (click a node row to show its strategy, click an action badge to walk to that action's node), the strategy matrix and a client-side made-hand category breakdown. The bridge is a local HTTP wrapper; the reference implementation is **`cuda-poker-solver/python/upi_bridge.py`** (submodule) driving the pps GPU postflop solver — run it with the env that has the built `pps` native module (stdlib HTTP server, no web-framework deps). Sizes are sent as `set_bet_sizes <street> <OOP|IP> bets=<csv> raises=<csv>` (percent-of-pot, keyed because a bare csv cannot be split into bets/raises); the bridge builds the tree from the FLOP OOP sizes (the pps engine takes a single bet config for the whole tree — bet fracs are %/100, a raise % becomes a multiple of the first bet size, min 2). Assumed contract — `POST {url}/upi` with `{"commands": string[]}` returning `{"responses": string[]}` (sequential, same solver state, `ERROR` lines preserved; bridge must answer CORS preflights; `show_children_actions` normalized to one action token per line, `show_strategy` to one 1326-float line per child in child order; the solve is chunked — the frontend loops `go_chunk <chunk_seconds> <total_seconds>`, each call returning one JSON progress line `{"iters","active","budget","done","nodes"}` until `done`, so the CALCULATE progress bar is fed between HTTP calls and a dropped call retries against a warm-start-identical schedule; the bridge also serves `GET /ws` — a WebSocket live-progress endpoint (stdlib RFC 6455) pushing `phase`/`progress`/`results` JSON frames during solves, ~0.5s slices, which the page consumes for per-iteration bar ticks and a liveness dot; there is no connect flow — CALCULATE tries the bridge and falls back to the demo solution when it is unreachable). Node ids are action paths: `r:0` is the root, a child is `<parent>:<token>`; chance deals are transparent — the bridge follows one representative runout per deal, so `show_node` reports the node's actual board. Default endpoint `http://127.0.0.1:8500`, persisted in localStorage. https pages may call http://localhost (potentially-trustworthy origin), so the deployed site can drive a local bridge.
