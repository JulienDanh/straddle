...
description: >
  ...
  Reads .md files from the private submodule's extracted/ (straddle-solutions/extracted/) and outputs a single flashcards.txt file
...
Convert the structured `.md` files in `straddle-solutions/extracted/` into a single
`flashcards.txt` file using `question;answer` format (semicolon separator).

## Source

The extracted `.md` files in `straddle-solutions/extracted/` — frontmatter + structured
...
1. Read all `.md` files in `straddle-solutions/extracted/` (there are 28).
