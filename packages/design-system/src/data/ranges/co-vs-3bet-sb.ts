// CO facing a SB 3-bet of the open, grouped by stack depth for
// RangeBrowser. Data lives in ranges/data/co/vs-3bet-sb.json. After
...
// 3-bet sizes from the SB defend entry (sb/vs-co.json) — both vary
// with depth and solution type.
import type { StoredRange } from './types'
import { materializeLine, preflopUrl, ICM_GAMETYPE } from './types'
import data from '../../../../ranges/data/co/vs-3bet-sb.json'
import opener from '../../../../ranges/data/co/rfi.json'
import defender from '../../../../ranges/data/sb/vs-co.json'
...
const line = (stack: number, type = 'cEV') =>
  `F-F-F-F-R${openSize(stack, type)}-FR${threebet(stack, type)}-F`

/** CO facing a SB 3-bet, ChipEV */
export const CO_VS_3BET_SB_CEV: StoredRange[] = materializeLine(
  { ...data, stacks: data.stacks.filter((e) => e.type === 'cEV') },
  (stack) => preflopUrl(stack, line(stack), 9))

/** CO facing a SB 3-bet on the ICM bubble (200-man) */
export const CO_VS_3BET_SB_ICM: StoredRange[] = materializeLine(
  { ...data, stacks: data.stacks.filter((e) => e.type === 'ICM') },
  (stack) => preflopUrl(stack, line(stack, 'ICM'), 9, ICM_GAMETYPE))
export const CO_VS_3BET_SB: StoredRange[] = [...CO_VS_3BET_SB_CEV, ...CO_VS_3BET_SB_ICM]
