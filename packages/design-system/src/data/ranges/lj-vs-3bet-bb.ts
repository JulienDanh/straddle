// LJ facing a BB 3-bet of the open, grouped by stack depth for
// RangeBrowser. Data lives in ranges/data/lj/vs-3bet-bb.json. After
...
// 3-bet sizes from the BB defend entry (bb/vs-lj.json) — both vary
// with depth and solution type.
import type { StoredRange } from './types'
import { materializeLine, preflopUrl, ICM_GAMETYPE } from './types'
import data from '../../../../ranges/data/lj/vs-3bet-bb.json'
import opener from '../../../../ranges/data/lj/rfi.json'
import defender from '../../../../ranges/data/bb/vs-lj.json'
...
const line = (stack: number, type = 'cEV') =>
  `F-F-R${openSize(stack, type)}-F-F-F-FR${threebet(stack, type)}`

/** LJ facing a BB 3-bet, ChipEV */
export const LJ_VS_3BET_BB_CEV: StoredRange[] = materializeLine(
  { ...data, stacks: data.stacks.filter((e) => e.type === 'cEV') },
  (stack) => preflopUrl(stack, line(stack), 9))

/** LJ facing a BB 3-bet on the ICM bubble (200-man) */
export const LJ_VS_3BET_BB_ICM: StoredRange[] = materializeLine(
  { ...data, stacks: data.stacks.filter((e) => e.type === 'ICM') },
  (stack) => preflopUrl(stack, line(stack, 'ICM'), 9, ICM_GAMETYPE))
export const LJ_VS_3BET_BB: StoredRange[] = [...LJ_VS_3BET_BB_CEV, ...LJ_VS_3BET_BB_ICM]
