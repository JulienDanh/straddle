// CO defense vs a UTG1 open, grouped by stack depth for RangeBrowser.
// Data lives in ranges/data/co/vs-utg1.json (CO acts after
// the UTG1 open, history_spot 5). The open size is read from
...
import data from '../../../../ranges/data/co/vs-utg1.json'
...
const line = (stack: number, type = 'cEV') =>
  `F-R${openSize(stack, type)}-F-F`

/** CO defense vs a UTG1 open, ChipEV */
export const CO_VS_UTG1_CEV: StoredRange[] = materializeLine(
  { ...data, stacks: data.stacks.filter((e) => e.type === 'cEV') },
  (stack) => preflopUrl(stack, line(stack), 5))

/** CO defense vs a UTG1 open on the ICM bubble (200-man) */
export const CO_VS_UTG1_ICM: StoredRange[] = materializeLine(
  { ...data, stacks: data.stacks.filter((e) => e.type === 'ICM') },
  (stack) => preflopUrl(stack, line(stack, 'ICM'), 5, ICM_GAMETYPE))
export const CO_VS_UTG1: StoredRange[] = [...CO_VS_UTG1_CEV, ...CO_VS_UTG1_ICM]
