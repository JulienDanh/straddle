// LJ facing a BTN 3-bet of the open, grouped by stack depth for
// RangeBrowser. Data lives in ranges/data/lj/vs-3bet-btn.json. After
...
// 3-bet sizes from the BTN defend entry (btn/vs-lj.json) — both vary
// with depth and solution type.
import type { StoredRange } from './types'
import { materializeLine, preflopUrl, ICM_GAMETYPE } from './types'
import data from '../../../../ranges/data/lj/vs-3bet-btn.json'
import opener from '../../../../ranges/data/lj/rfi.json'
import defender from '../../../../ranges/data/btn/vs-lj.json'
...
const line = (stack: number, type = 'cEV') =>
  `F-F-R${openSize(stack, type)}-F-FR${threebet(stack, type)}-F-F`

/** LJ facing a BTN 3-bet, ChipEV */
export const LJ_VS_3BET_BTN_CEV: StoredRange[] = materializeLine(
  { ...data, stacks: data.stacks.filter((e) => e.type === 'cEV') },
  (stack) => preflopUrl(stack, line(stack), 9))

/** LJ facing a BTN 3-bet on the ICM bubble (200-man) */
export const LJ_VS_3BET_BTN_ICM: StoredRange[] = materializeLine(
  { ...data, stacks: data.stacks.filter((e) => e.type === 'ICM') },
  (stack) => preflopUrl(stack, line(stack, 'ICM'), 9, ICM_GAMETYPE))
export const LJ_VS_3BET_BTN: StoredRange[] = [...LJ_VS_3BET_BTN_CEV, ...LJ_VS_3BET_BTN_ICM]
