// UTG1 facing a BTN 3-bet of the open, grouped by stack depth for
// RangeBrowser. Data lives in ranges/data/utg1/vs-3bet-btn.json. After
...
// 3-bet sizes from the BTN defend entry (btn/vs-utg1.json) — both vary
// with depth and solution type.
import type { StoredRange } from './types'
import { materializeLine, preflopUrl, ICM_GAMETYPE } from './types'
import data from '../../../../ranges/data/utg1/vs-3bet-btn.json'
import opener from '../../../../ranges/data/utg1/rfi.json'
import defender from '../../../../ranges/data/btn/vs-utg1.json'
...
const line = (stack: number, type = 'cEV') =>
  `F-R${openSize(stack, type)}-F-F-FR${threebet(stack, type)}-F-F`

/** UTG1 facing a BTN 3-bet, ChipEV */
export const UTG1_VS_3BET_BTN_CEV: StoredRange[] = materializeLine(
  { ...data, stacks: data.stacks.filter((e) => e.type === 'cEV') },
  (stack) => preflopUrl(stack, line(stack), 9))

/** UTG1 facing a BTN 3-bet on the ICM bubble (200-man) */
export const UTG1_VS_3BET_BTN_ICM: StoredRange[] = materializeLine(
  { ...data, stacks: data.stacks.filter((e) => e.type === 'ICM') },
  (stack) => preflopUrl(stack, line(stack, 'ICM'), 9, ICM_GAMETYPE))
export const UTG1_VS_3BET_BTN: StoredRange[] = [...UTG1_VS_3BET_BTN_CEV, ...UTG1_VS_3BET_BTN_ICM]
