// HJ facing a BTN 3-bet of the open, grouped by stack depth for
// RangeBrowser. Data lives in ranges/data/hj/vs-3bet-btn.json. After
...
// 3-bet sizes from the BTN defend entry (btn/vs-hj.json) — both vary
// with depth and solution type.
import type { StoredRange } from './types'
import { materializeLine, preflopUrl, ICM_GAMETYPE } from './types'
import data from '../../../../ranges/data/hj/vs-3bet-btn.json'
import opener from '../../../../ranges/data/hj/rfi.json'
import defender from '../../../../ranges/data/btn/vs-hj.json'
...
const line = (stack: number, type = 'cEV') =>
  `F-F-F-R${openSize(stack, type)}-FR${threebet(stack, type)}-F-F`

/** HJ facing a BTN 3-bet, ChipEV */
export const HJ_VS_3BET_BTN_CEV: StoredRange[] = materializeLine(
  { ...data, stacks: data.stacks.filter((e) => e.type === 'cEV') },
  (stack) => preflopUrl(stack, line(stack), 9))

/** HJ facing a BTN 3-bet on the ICM bubble (200-man) */
export const HJ_VS_3BET_BTN_ICM: StoredRange[] = materializeLine(
  { ...data, stacks: data.stacks.filter((e) => e.type === 'ICM') },
  (stack) => preflopUrl(stack, line(stack, 'ICM'), 9, ICM_GAMETYPE))
export const HJ_VS_3BET_BTN: StoredRange[] = [...HJ_VS_3BET_BTN_CEV, ...HJ_VS_3BET_BTN_ICM]
