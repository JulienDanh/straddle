...
range store's combo-line format.
  ...
  python3 packages/ranges/scripts/gw_to_store.py flop .scratch/k83-fetched.json
      --parent utg/rfi.json --stack 40
  python3 packages/ranges/scripts/gw_to_store.py preflop .scratch/utg40-fetched.json
...
STORE = pathlib.Path(__file__).resolve().parent.parent / "data"
MIN_FREQ = 0.00005
...
def flop(data, parent_rel, stack):
    combos = combo_order()
    ...
    entry = next((s for s in parent["stacks"] if s["stack"] == stack
                  and "raise" in s.get("actions", {})), None)
    if entry is None:
...
def main():
    ...
    out = flop(data, args.parent, args.stack) if args.mode == "flop" else preflop(data)
