import { Section, Callout, Code, Action, Subhead, DataTable } from '@poker/design-system/src/components/ui'
import { RangeBrowser } from '@poker/design-system/src/components/RangeBrowser'
import { UTG_RFI_EQUAL, BTN_RFI_EQUAL } from '@poker/design-system/src/data/ranges'
...
export function BM1Page() {
  return (
    <Section title="ICM vs ChipEV — Preflop Adjustments">
      ...
      <>
              <Subhead>ChipEV vs ICM-Adjusted</Subhead>
...
<p className="mt-6 mb-1 text-sm text-muted">Solver versions of the shift — each panel carries both solutions; toggle cEV / ICM to see the bubble tighten the open (UTG drops suited connectors and low pairs for ace blockers; BTN opens 49% under ICM vs wider in chips):</p>
              <RangeBrowser ranges={UTG_RFI_EQUAL} />
              <RangeBrowser ranges={BTN_RFI_EQUAL} /> A2s opens where Q9s folds. A9o opens where T9s folds. You're blocking 3-bet bluffs, not playing postflop — because opponents 3-bet/fold, not cold call.</Callout>

              <Subhead>The five shifts</Subhead>
